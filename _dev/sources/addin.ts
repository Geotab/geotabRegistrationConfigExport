...
class Addin {
  ...
  private readonly saveBtn = document.getElementById(
    "saveButton",
  ) as HTMLButtonElement;
  private readonly exportAllAddinsCheckbox: HTMLInputElement =
    document.getElementById("export_all_addins_checkbox") as HTMLInputElement;
  private readonly exportAllZonesCheckbox: HTMLInputElement =
    document.getElementById("export_all_zones_checkbox") as HTMLInputElement;
  ...
  private addNewGroups(groups: string[], data: IImportData): Promise<any> {
    ...
    let groupsData = this.groupsBuilder.getGroupsData(groups, true),
      newGroupsUsers = getUniqueEntities(
  ...
  private addNewNotificationTemplates(
    notificationTemplatesIds: string[],
    data: IImportData,
  ) {
    ...
    let notificationTemplatesData = notificationTemplatesIds.reduce(
      (data: any[], templateId: string) => {
        let templateData =
          this.distributionListsBuilder.getNotificationTemplateData(templateId);
  ...
  private getEntityDependencies(entity: IEntity, entityType: TEntityType) {
    ...
    switch (entityType) {
      case "devices":
        ...
        entity["workTime"].id &&
          (entityDependencies.workTimes = [entity["workTime"].id]);
        break;
      case "users":
        entityDependencies.groups = this.getEntytiesIds(
          entity["companyGroups"]
            .concat(entity["driverGroups"])
            .concat(entity["privateUserGroups"])
            .concat(entity["reportGroups"]),
        );
      ...
      case "zones":
        ...
        break;
      case "workTimes":
        entity["holidayGroup"].groupId &&
          (entityDependencies.workHolidays = [entity["holidayGroup"].groupId]);
        break;
      case "groupFilters":
        entityDependencies.groups = [
          ...getGroupFilterGroups(
            (entity as IGroupFilter).groupFilterCondition,
          ).values(),
        ];
        break;
      default:
  ...
  private applyToEntities<T>(
    entitiesList: IDependencies,
    initialValue,
    func: (
      result,
      entity,
    ...
    return Object.keys(entitiesList).reduce(
      (result: T, entityType: string, typeIndex: number) => {
  ...
  private resolveDependencies(dependencies: IDependencies, data: IImportData) {
    let getData = (entitiesList: IDependencies): Promise<IImportData> => {
      let entityRequestTypes = {
        ...
        requests: any = this.applyToEntities(
          entitiesList,
          {},
      ...
      return this.addNewGroups(entitiesList.groups || [], data).then(() => {
        ...
        return new Promise<IImportData>((resolve, reject) => {
          let requestEntities = Object.keys(requests),
            requestsArray = requestEntities.reduce(
          ...
          multiCall(this.api, requestsArray).then((response) => {
            let newGroups: string[] = [],
              ...
              exportedData: any = this.applyToEntities(
                requests,
                {},
                (
                  result,
                  ...
                  let items =
                    requestsArray.length > 1
                      ? response[overallIndex]
                      : response;
                  items.forEach((item) => {
                    item = item[0] || item;
                    !result[entityType] && (result[entityType] = []);
                    if (
                      entityType === "workHolidays" &&
                      (!item.holidayGroup ||
                        (entitiesList.workHolidays || []).indexOf(
                    ...
                    if (entityType === "securityGroups") {
                      if (
                        (entitiesList.securityGroups || []).indexOf(item.id) >
                        -1
                      ) {
                    ...
                    newDependencies = this.applyToEntities(
                      ...
                      (result, entityId, entityType) => {
                        !result[entityType] && (result[entityType] = []);
                        result[entityType] = mergeUnique(result[entityType], [
                          entityId,
                        ]);
                        return result;
                      },
            ...
            newDependencies = Object.keys(newDependencies).reduce(
              (result, dependencyName) => {
                let entities = newDependencies[dependencyName],
                  exported = (exportedData[dependencyName] || []).map(
                    (entity) => entity.id,
                  );
            ...
            exportedData.securityGroups &&
              (exportedData.securityGroups = exportedData.securityGroups.reduce(
                (result, group) => {
  ...
  private readonly toggleWaiting = (isStart = false) => {
    if (isStart) {
      ...
      this.waiting.start(
        (document.getElementById("addinContainer") as HTMLElement)
  ...
  private showEntityMessage(
    ...
    if (qty) {
      qty > 1 && (entityName += "s");
      let hasItemsMessageTemplate = (
        document.getElementById("hasItemsMessageTemplate") as HTMLElement
      ).innerHTML;
      blockEl.innerHTML = hasItemsMessageTemplate
  ...
  private showSystemSettingsMessage(block: HTMLElement, isIncluded: boolean) {
    ...
    if (isIncluded) {
      blockEl.innerHTML =
        "You have chosen <span class='bold'>to include</span> system settings.";
    } else {
      blockEl.innerHTML =
        "You have chosen <span class='bold'>not to include</span> system settings.";
    }
  }
  ...
  render() {
    ...
    let mapMessageTemplate: string = (
      ...
      groupsBlock: HTMLElement = document.getElementById(
        "exportedGroups",
      ) as HTMLElement,
      securityClearancesBlock: HTMLElement = document.getElementById(
        "exportedSecurityClearances",
      ) as HTMLElement,
      rulesBlock: HTMLElement = document.getElementById(
        "exportedRules",
      ) as HTMLElement,
      reportsBlock: HTMLElement = document.getElementById(
        "exportedReports",
      ) as HTMLElement,
      dashboardsBlock: HTMLElement = document.getElementById(
        "exportedDashboards",
      ) as HTMLElement,
      addinsBlock: HTMLElement = document.getElementById(
        "exportedAddins",
      ) as HTMLElement,
      mapBlockDescription: HTMLElement = document.querySelector(
        "#exportedMap .description",
      ) as HTMLElement,
      // usersBlock: HTMLElement = document.getElementById("exportedUsers"),
      zonesBlock: HTMLElement = document.getElementById(
        "exportedZones",
      ) as HTMLElement,
    ...
    const zonesQtyPromise =
      this.exportAllZonesCheckbox.checked == true
        ? this.zoneBuilder.getQty()
        : Promise.resolve(0);
    return zonesQtyPromise
      .then((zonesQty) => {
      ...
      .then((results) => {
        ...
        this.data.distributionLists =
          this.distributionListsBuilder.getRulesDistributionLists(
        ...
        customMap =
          this.data.misc &&
          this.miscBuilder.getMapProviderData(this.data.misc.mapProvider.value);
        ...
        distributionListsDependencies =
          this.distributionListsBuilder.getDependencies(
            this.data.distributionLists,
          );
      ...
      .then(() => {
        ...
        let mapProvider =
          this.data.misc &&
          this.miscBuilder.getMapProviderName(this.data.misc.mapProvider.value);
